...
object ActorApp extends App{
    ...
    val orderActor = system.actorOf(Props(new OrderOperationActor(orderList)), name = "orderActor")
   ...
   /* implicit val timeout = Timeout(5 seconds)
    ...
    val result = Await.result(future, timeout.duration).asInstanceOf[ListBuffer[Order]]
    println(result)*/
    ...
    //val result1 = Await.result(future, timeout.duration).asInstanceOf[ListBuffer[Order]]
    println(orderList)
